...
from helpers import is_admin, to_html_simple
...
async def _show_preview(message: Message, uid: int) -> None:
    ...
    photo = st.get("photo")
    pin = bool(st.get("pin"))
    users_cnt = _users_count()

    preview_caption = (
        "👁 <b>Предпросмотр рассылки</b>\n"
        "━━━━━━━━━━━━━━━\n\n"
        f"{to_html_simple(text)}\n\n"
        "━━━━━━━━━━━━━━━\n"
    ...
    if photo:
        await message.answer_photo(photo, caption=preview_caption, parse_mode="HTML", reply_markup=bcw_preview_kb())
    else:
        await message.answer(preview_caption, parse_mode="HTML", reply_markup=bcw_preview_kb())
...
async def broadcast_wizard_cb(call: CallbackQuery):
    ...
    if action == "send":
        ...
        await do_broadcast(call.message, uid, label, text, photo=photo, pin=pin)
        return
